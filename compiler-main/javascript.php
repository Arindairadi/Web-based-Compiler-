<?php
// javascript.php - JavaScript Bytecode & V8 Engine Visualizer
...
ini_set('error_log', 'js_errors.log');
...
function handleApiRequest($endpoint) {
    ...
    try {
        switch ($endpoint) {
            case 'compile': $result = handleJsCompile(); break;
...
function handleJsCompile() {
    ...
    $syntaxResult = simulateJsAST($source_code);
    ...
    $semanticResult = simulateJsSymbolTable($source_code);
    ...
    $bytecodeResult = simulateJsBytecode($source_code);
    ...
    $executionResult = simulateJsExecution($source_code);
    $result['stages'][] = ['name' => 'Interpretation (V8)', 'status' => $executionResult['has_errors'] ? 'failed' : 'completed', 'duration' => rand(180,450), 'execution' => $executionResult['execution'], 'errors' => $executionResult['errors']];
    ...
    if ($userId) logActivity($userId, 'compile', "Compiled/interpreted JavaScript code, session: $session_id, errors: $errorsCount");
...
function handleVisualize() {
    ...
    $viz_data = generateJsVisualizationData($result, $stage, $view);
    return $viz_data;
}
...
function handleDownload() {
    ...
    switch ($type) {
        case 'source':
            $file = "{$tmp_dir}/script.js";
            $filename = "script_{$session_id}.js";
        ...
        case 'bytecode':
            ...
            $filename = "bytecode_{$session_id}.jsc";
            $content_type = 'text/plain';
            header('Content-Type: ' . $content_type);
            header('Content-Disposition: attachment; filename="' . $filename . '"');
            header('Content-Length: ' . strlen($content));
...
function simulateJsTokens($code) {
    $tokens = []; $errors = [];
    $code = preg_replace('/\/\/.*$/m', '', $code);
    $code = preg_replace('/\/\*.*?\*\//s', '', $code);
    $lines = explode("\n", $code);
    $keywords = ['let', 'const', 'var', 'function', 'if', 'else', 'for', 'while', 'do', 'switch', 'case', 'break', 'continue', 'return', 'try', 'catch', 'finally', 'throw', 'class', 'extends', 'super', 'new', 'this', 'typeof', 'instanceof', 'delete', 'in', 'void', 'yield', 'await', 'async', 'export', 'import', 'default', 'true', 'false', 'null', 'undefined'];
...
function simulateJsAST($code) {
    ...
    if (strpos($code, 'function') !== false) $ast['body'][] = ['type' => 'FunctionDeclaration', 'name' => 'example'];
    if (strpos($code, 'console.log') !== false) $ast['body'][] = ['type' => 'ExpressionStatement', 'expression' => ['type' => 'CallExpression', 'callee' => ['type' => 'MemberExpression', 'object' => ['type' => 'Identifier', 'name' => 'console'], 'property' => ['type' => 'Identifier', 'name' => 'log']]]];
    return ['ast' => $ast, 'errors' => $errors, 'has_errors' => !empty($errors)];
}

function simulateJsSymbolTable($code) {
    $symbols = ['functions' => [], 'variables' => []];
    $errors = [];
    if (preg_match('/function\s+(\w+)/', $code, $m)) $symbols['functions'][] = $m[1];
    if (preg_match_all('/\b(let|const|var)\s+(\w+)/', $code, $m)) foreach ($m[2] as $var) $symbols['variables'][] = $var;
    return ['symbol_table' => $symbols, 'errors' => $errors, 'has_errors' => false];
}

function simulateJsBytecode($code) {
    $bytecode = [];
    $bytecode[] = '; JavaScript Bytecode (V8 Ignition)';
    $bytecode[] = '; Source: ' . substr($code, 0, 50) . '...';
    $bytecode[] = '';
    $bytecode[] = '[generated bytecode for function: main]';
    $bytecode[] = 'Parameter count 1';
    $bytecode[] = 'Frame size 8';
    $bytecode[] = '         0x0 LdaConstant [0]';
    $bytecode[] = '         0x2 Star r0';
    $bytecode[] = '         0x4 LdaConstant [1]';
    $bytecode[] = '         0x6 Star r1';
    if (strpos($code, '+') !== false) {
        $bytecode[] = '         0x8 Lda r0';
        $bytecode[] = '        0x10 Add r1';
        $bytecode[] = '        0x12 Star r2';
    }
    if (strpos($code, 'console.log') !== false) {
        $bytecode[] = '        0x14 LdaGlobal [2] (console)';
        $bytecode[] = '        0x16 Star r3';
        $bytecode[] = '        0x18 LdaNamedProperty r3, [3] (log)';
        $bytecode[] = '        0x20 Star r4';
        $bytecode[] = '        0x22 Lda r2';
        $bytecode[] = '        0x24 CallProperty r4, r3, [1]';
    }
    $bytecode[] = '        0x26 LdaUndefined';
    $bytecode[] = '        0x28 Return';
...
function simulateJsExecution($code) {
    ...
    if (strpos($code, 'console.log') !== false) {
        ...
        else $output = "Hello from JavaScript!\n";
    }
...
function generateJsVisualizationData($result, $stage, $view) {
    $nodes = []; $edges = [];
    if ($view === 'pipeline') {
        $stages = [
            ...
            ['id' => 'v8', 'name' => 'Interpretation (V8)', 'x' => 18, 'y' => 0, 'z' => 0],
        ];
        ...
        if (isset($result['outputs']['ast'])) { $ast = $result['outputs']['ast']; $node_id=0; createJsASTNodes($ast, $nodes, $edges, $node_id, 0,10,0,-1); }
    } elseif ($view === 'memory') {
...
function createJsASTNodes($node, &$nodes, &$edges, &$node_id, $x, $y, $z, $parent_id) {
    $current_id = $node_id++;
    ...
    foreach ($node as $key => $value) if (is_array($value) && isset($value['type'])) {
        ...
        createJsASTNodes($value, $nodes, $edges, $node_id, $child_x, $child_y, $child_z, $current_id);
        $child_index++;
    }
    return $current_id;
}

// ---------- HTML output (standalone, same style as others) ----------
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>3D JavaScript Interpreter Visualizer - Final Project</title>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/controls/OrbitControls.min.js"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=JetBrains+Mono:wght@300;400;500&display=swap" rel="stylesheet">
    <style>
        /* Same as php.php styles (identical) */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .js-key-item { display: flex; align-items: center; gap: 4px; font-size: 0.8rem; color: #8892b0; }
        .js-key-item .key { background: rgba(100, 255, 218, 0.1); padding: 2px 6px; border-radius: 3px; font-family: 'JetBrains Mono', monospace; color: #64ffda; }
    </style>
</head>
<body>
    <div class="header">
        <div>
            <h1><i class="fab fa-js"></i> 3D JavaScript Interpreter Visualizer
                <span class="interpreted-badge"><i class="fas fa-magic"></i> Interpreted Language</span>
            </h1>
            <div class="subtitle">Interactive visualization of JavaScript compilation to bytecode & execution by V8 Engine</div>
        </div>
        <div class="course-info">
            <span class="university">COMPUTER SCIENCE FINAL PROJECT - JAVASCRIPT VERSION</span>
        </div>
        <div class="user-info">
            <?php if (isLoggedIn()): ?>
        ...
        <div class="main-content">
            ...
            <div class="panel controls-panel">
                <div class="panel-header">
                    ...
                    <h2>JavaScript Compilation & Interpretation Controls</h2>
                </div>
                <div class="controls-scroll-container">
                    ...
                    <div class="control-group">
                        ...
                        <select id="stage-select">
                            ...
                            <option value="v8">Interpretation (V8)</option>
                        </select>
                    </div>
                    ...
                    <div class="js-key">
                        <div class="js-key-item"><span class="key">let/const</span> Variable</div>
                        <div class="js-key-item"><span class="key">function</span> Function</div>
                        <div class="js-key-item"><span class="key">if/else</span> Conditional</div>
                        <div class="js-key-item"><span class="key">for/while</span> Loop</div>
                        <div class="js-key-item"><span class="key">console.log</span> Output</div>
                    </div>
                    ...
                    <div class="panel-header" style="margin-top: 20px;">
                        ...
                        <h2>JavaScript Source Code Editor</h2>
                    </div>
                    <div class="code-editor">
                        <div class="code-editor-header">
                            <h3><i class="fab fa-js"></i> JavaScript Code</h3>
                        ...
                        <textarea id="source-code" spellcheck="false">// JavaScript is interpreted - source code → bytecode → V8 execution
let x = 10;
let y = 20;
let result = x + y;
...
if (result > 25) {
    ...
    console.log("Result is 25 or less");
}</textarea>
                    </div>
                    ...
                    <div class="status-bar">
                        <div id="status-message"><i class="fas fa-info-circle"></i> Ready to interpret JavaScript code</div>
            ...
            <div class="panel output-panel" id="output-panel">
                ...
                <div class="panel-header"><i class="fas fa-terminal"></i><h2>JavaScript Compilation & Execution Output</h2></div>
                <div class="output-scroll-container">
                    <div class="output-tabs">
                        ...
                        <button class="output-tab" data-output="bytecode"><i class="fas fa-microchip"></i> Bytecode</button>
                    ...
                    <div id="stage-info" class="stage-info">
                        <p><i class="fas fa-mouse-pointer"></i> JavaScript is an interpreted language: source → bytecode → V8 execution.</p>
                        <div style="margin-top:12px; padding:12px; background:rgba(100,255,218,0.05); border-radius:5px; border:1px dashed rgba(100,255,218,0.2);">
                            ...
                            <p style="font-size:0.8rem; color:#8892b0;">Unlike compiled languages (C, Go), JavaScript code is first compiled to bytecode then executed by the V8 engine (JIT compilation).</p>
                        </div>
                    </div>
                    ...
                    <div class="export-buttons">
                        ...
                        <button id="download-bytecode" class="btn secondary"><i class="fas fa-download"></i> Bytecode</button>
    ...
    <div class="footer">
        <div class="footer-content">
            <div class="course-info">Final Year Project - Computer Science Department | JavaScript Bytecode & V8 3D Visualization</div>
            ...
            <a href="https://github.com/Agabaofficial/javascript-compiler-3d-visualizer" target="_blank" class="github-link"><i class="fab fa-github"></i> View on GitHub</a>
        </div>
    </div>
    ...
    <script>
        ...
        const jsExamples = <?php echo json_encode([
            'simple' => '// Simple math\nlet x = 10;\nlet y = 20;\nlet result = x + y;\nconsole.log("Result:", result);',
            'loop' => '// For loop\nfor (let i = 0; i < 5; i++) {\n    console.log("Iteration", i);\n}',
        ...
        class JsInterpreterVisualizer3D {
            constructor() {
                this.sessionId = null;
            ...
            async compile() {
                ...
                if (!code.trim()) { this.showStatus('Please enter JavaScript code', 'error'); return; }
                ...
                try {
                    ...
                    this.showStatus(data.success ? 'JavaScript interpretation successful!' : 'Completed with warnings', data.success ? 'success' : 'warning');
            ...
            updateOutputs(data) {
                ...
                if (data.outputs?.bytecode) document.getElementById('bytecode-output').textContent = data.outputs.bytecode.join('\n');
                ...
                if (data.stages?.length) {
                    let html = '<h3 style="color:#64ffda;">JavaScript Pipeline</h3>';
            ...
            reset(){
                ...
                document.getElementById('source-code').value = `// JavaScript is interpreted - source code → bytecode → V8 execution\nlet x = 10;\nlet y = 20;\nlet result = x + y;\n\nconsole.log("Result:", result);\n\nfor (let i = 0; i < 5; i++) {\n    console.log("Iteration", i);\n}\n\nif (result > 25) {\n    console.log("Result is greater than 25");\n} else {\n    console.log("Result is 25 or less");\n}`;
                ...
                document.getElementById('stage-info').innerHTML = `<p><i class="fas fa-mouse-pointer"></i> JavaScript is an interpreted language: source → bytecode → V8 execution.</p><div style="margin-top:12px; padding:12px; background:rgba(100,255,218,0.05); border-radius:5px; border:1px dashed rgba(100,255,218,0.2);"><p style="color:#64ffda;"><i class="fas fa-lightbulb"></i> <strong>Key Distinction:</strong></p><p style="font-size:0.8rem; color:#8892b0;">Unlike compiled languages (C, Go), JavaScript code is first compiled to bytecode then executed by the V8 engine (JIT compilation).</p></div>`;
                this.clearScene(); this.createDefaultVisualization(); this.updateProgress(0); this.showStatus('Ready to interpret JavaScript code');
            }
            ...
            takeScreenshot(){ let link=document.createElement('a'); link.href=this.renderer.domElement.toDataURL('image/png'); link.download=`js_viz_${Date.now()}.png`; link.click(); }
            async download(type){
                ...
                try{
                    ...
                    a.href=url; a.download=type==='bytecode'?`bytecode_${this.sessionId}.jsc`:type==='ast'?`ast_${this.sessionId}.json`:`script_${this.sessionId}.js`;
            ...
            loadExample(exampleId){
                if (jsExamples[exampleId]) {
                    document.getElementById('source-code').value = jsExamples[exampleId];
                    this.showStatus(`Loaded JavaScript example: ${exampleId}`, 'success');
                }
        ...
        document.addEventListener('DOMContentLoaded',()=>{ window.jsViz = new JsInterpreterVisualizer3D(); });
    </script>
</body>
</html>