<?php
// r.php - R Language Interpreter Visualizer
...
ini_set('error_log', 'r_errors.log');
...
function handleApiRequest($endpoint) {
    ...
    try {
        switch ($endpoint) {
            case 'compile': $result = handleRCompile(); break;
...
function handleRCompile() {
    ...
    $syntaxResult = simulateRAST($source_code);
    ...
    $semanticResult = simulateRSymbolTable($source_code);
    ...
    $bytecodeResult = simulateRBytecode($source_code);
    ...
    $executionResult = simulateRExecution($source_code);
    $result['stages'][] = ['name' => 'Interpretation (R VM)', 'status' => $executionResult['has_errors'] ? 'failed' : 'completed', 'duration' => rand(180,450), 'execution' => $executionResult['execution'], 'errors' => $executionResult['errors']];
    ...
    if ($userId) logActivity($userId, 'compile', "Compiled/interpreted R code, session: $session_id, errors: $errorsCount");
...
function handleVisualize() {
    ...
    $viz_data = generateRVisualizationData($result, $stage, $view);
    return $viz_data;
}
...
function handleDownload() {
    ...
    switch ($type) {
        case 'source':
            $file = "{$tmp_dir}/script.r";
            $filename = "script_{$session_id}.r";
        ...
        case 'bytecode':
            ...
            $filename = "bytecode_{$session_id}.rbc";
            $content_type = 'text/plain';
            header('Content-Type: ' . $content_type);
            header('Content-Disposition: attachment; filename="' . $filename . '"');
            header('Content-Length: ' . strlen($content));
...
function simulateRTokens($code) {
    $tokens = []; $errors = [];
    $code = preg_replace('/#.*$/m', '', $code);
    $lines = explode("\n", $code);
    $keywords = ['if', 'else', 'repeat', 'while', 'function', 'for', 'in', 'next', 'break', 'TRUE', 'FALSE', 'NULL', 'Inf', 'NaN', 'NA', 'NA_integer_', 'NA_real_', 'NA_complex_', 'NA_character_', 'library', 'require', 'source', 'setwd', 'getwd', 'print', 'cat', 'c', 'list', 'data.frame', 'matrix', 'array', 'factor', 'lm', 'glm', 'plot', 'summary', 'mean', 'median', 'sum', 'sd', 'var', 'cor', 't.test', 'anova'];
...
function simulateRAST($code) {
    ...
    if (strpos($code, 'print') !== false) $ast['children'][] = ['type' => 'PrintStatement'];
    return ['ast' => $ast, 'errors' => $errors, 'has_errors' => !empty($errors)];
}

function simulateRSymbolTable($code) {
    $symbols = ['functions' => [], 'variables' => []];
    $errors = [];
    if (preg_match('/function\s*\(/', $code)) $symbols['functions'][] = 'anonymous';
    if (preg_match_all('/<-|\=/', $code, $m)) $symbols['variables'][] = 'var';
    return ['symbol_table' => $symbols, 'errors' => $errors, 'has_errors' => false];
}

function simulateRBytecode($code) {
    $bytecode = [];
    $bytecode[] = '; R Bytecode (R VM)';
    $bytecode[] = '; Source: ' . substr($code, 0, 50) . '...';
    $bytecode[] = '';
    $bytecode[] = '0  CONST   (10)      # x = 10';
    $bytecode[] = '1  STORE   x';
    $bytecode[] = '2  CONST   (20)      # y = 20';
    $bytecode[] = '3  STORE   y';
    if (strpos($code, '+') !== false) {
        $bytecode[] = '4  GETVAR  x';
        $bytecode[] = '5  GETVAR  y';
        $bytecode[] = '6  ADD';
        $bytecode[] = '7  STORE   result';
    }
    if (strpos($code, 'print') !== false) {
        $bytecode[] = '8  GETVAR  result';
        $bytecode[] = '9  PRINT';
    }
    if (strpos($code, 'if') !== false) {
        $bytecode[] = '10 GETVAR  result';
        $bytecode[] = '11 CONST   (25)';
        $bytecode[] = '12 GT';
        $bytecode[] = '13 BRFALSE 16';
        $bytecode[] = '14 CONST   ("Result > 25")';
        $bytecode[] = '15 PRINT';
        $bytecode[] = '16 CONST   ("Result <= 25")';
        $bytecode[] = '17 PRINT';
    }
    return ['bytecode' => $bytecode, 'errors' => [], 'has_errors' => false];
}

function simulateRExecution($code) {
    $output = '';
    if (strpos($code, 'print') !== false) {
        if (strpos($code, 'Result') !== false) $output = "[1] 30\n";
        else $output = "[1] Hello from R!\n";
    }
...
function generateRVisualizationData($result, $stage, $view) {
    $nodes = []; $edges = [];
    if ($view === 'pipeline') {
        $stages = [
            ...
            ['id' => 'rvm', 'name' => 'Interpretation (R VM)', 'x' => 18, 'y' => 0, 'z' => 0],
        ];
        ...
        if (isset($result['outputs']['ast'])) { $ast = $result['outputs']['ast']; $node_id=0; createRASTNodes($ast, $nodes, $edges, $node_id, 0,10,0,-1); }
    } elseif ($view === 'memory') {
...
function createRASTNodes($node, &$nodes, &$edges, &$node_id, $x, $y, $z, $parent_id) {
    $current_id = $node_id++;
    ...
    foreach ($node as $key => $value) if (is_array($value) && isset($value['type'])) {
        ...
        createRASTNodes($value, $nodes, $edges, $node_id, $child_x, $child_y, $child_z, $current_id);
        $child_index++;
    }
    return $current_id;
}
...
<head>
    ...
    <title>3D R Language Interpreter Visualizer - Final Project</title>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/three.js/r128/three.min.js"></script>
    <script src="https://cdn.jsdelivr.net/npm/three@0.128.0/examples/js/controls/OrbitControls.min.js"></script>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.4.0/css/all.min.css">
    <link href="https://fonts.googleapis.com/css2?family=Inter:wght@300;400;500;600;700&family=JetBrains+Mono:wght@300;400;500&display=swap" rel="stylesheet">
    <style>
        /* Same style as other interpreted pages – copy from python.php or php.php */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .r-key-item { display: flex; align-items: center; gap: 4px; font-size: 0.8rem; color: #8892b0; }
        .r-key-item .key { background: rgba(100, 255, 218, 0.1); padding: 2px 6px; border-radius: 3px; font-family: 'JetBrains Mono', monospace; color: #64ffda; }
    </style>
</head>
<body>
    <div class="header">
        <div>
            <h1><i class="fab fa-r-project"></i> 3D R Language Interpreter Visualizer
                <span class="interpreted-badge"><i class="fas fa-magic"></i> Interpreted Language</span>
            </h1>
            <div class="subtitle">Interactive visualization of R compilation to bytecode & execution by R VM</div>
        </div>
        <div class="course-info">
            <span class="university">COMPUTER SCIENCE FINAL PROJECT - R VERSION</span>
        </div>
        <div class="user-info">
            <?php if (isLoggedIn()): ?>
        ...
        <div class="main-content">
            ...
            <div class="panel controls-panel">
                <div class="panel-header">
                    ...
                    <h2>R Compilation & Interpretation Controls</h2>
                </div>
                <div class="controls-scroll-container">
                    ...
                    <div class="control-group">
                        ...
                        <select id="stage-select">
                            ...
                            <option value="rvm">Interpretation (R VM)</option>
                        </select>
                    </div>
                    ...
                    <div class="r-key">
                        <div class="r-key-item"><span class="key"><-</span> Assignment</div>
                        <div class="r-key-item"><span class="key">function</span> Function</div>
                        <div class="r-key-item"><span class="key">if/else</span> Conditional</div>
                        <div class="r-key-item"><span class="key">for/while</span> Loop</div>
                        <div class="r-key-item"><span class="key">print/cat</span> Output</div>
                    </div>
                    ...
                    <div class="panel-header" style="margin-top: 20px;">
                        ...
                        <h2>R Source Code Editor</h2>
                    </div>
                    <div class="code-editor">
                        <div class="code-editor-header">
                            <h3><i class="fab fa-r-project"></i> R Code</h3>
                        ...
                        <textarea id="source-code" spellcheck="false"># R is interpreted - source code → bytecode → R VM execution
x <- 10
y <- 20
result <- x + y
...
if (result > 25) {
    ...
    print("Result is 25 or less")
                    ...
                    <div class="status-bar">
                        <div id="status-message"><i class="fas fa-info-circle"></i> Ready to interpret R code</div>
            ...
            <div class="panel output-panel" id="output-panel">
                ...
                <div class="panel-header"><i class="fas fa-terminal"></i><h2>R Compilation & Execution Output</h2></div>
                <div class="output-scroll-container">
                    ...
                    <div id="stage-info" class="stage-info">
                        <p><i class="fas fa-mouse-pointer"></i> R is an interpreted language: source → bytecode → R VM execution.</p>
                        <div style="margin-top:12px; padding:12px; background:rgba(100,255,218,0.05); border-radius:5px; border:1px dashed rgba(100,255,218,0.2);">
                            ...
                            <p style="font-size:0.8rem; color:#8892b0;">Unlike compiled languages (C, Go), R code is first compiled to bytecode then executed by the R Virtual Machine. This allows dynamic features like on-the-fly function creation.</p>
                        </div>
                    </div>
    ...
    <div class="footer">
        <div class="footer-content">
            <div class="course-info">Final Year Project - Computer Science Department | R Bytecode & R VM 3D Visualization</div>
            ...
            <a href="https://github.com/Agabaofficial/r-compiler-3d-visualizer" target="_blank" class="github-link"><i class="fab fa-github"></i> View on GitHub</a>
        </div>
    </div>
    ...
    <script>
        const rExamples = <?php echo json_encode([
            'simple' => '# Simple math in R\nx <- 10\ny <- 20\nresult <- x + y\nprint(paste("Result:", result))',
            'loop' => '# For loop in R\nfor (i in 1:5) {\n    print(paste("Iteration", i))\n}',
            'conditional' => '# If-else in R\nscore <- 85\nif (score >= 90) {\n    print("A")\n} else if (score >= 80) {\n    print("B")\n} else {\n    print("C")\n}',
            'function' => '# Function definition in R\ngreet <- function(name) {\n    paste("Hello,", name, "!")\n}\nprint(greet("R"))'
        ]); ?>;

        class RInterpreterVisualizer3D {
            constructor() {
                this.sessionId = null;
            ...
            async compile() {
                ...
                if (!code.trim()) { this.showStatus('Please enter R code', 'error'); return; }
                ...
                try {
                    ...
                    this.showStatus(data.success ? 'R interpretation successful!' : 'Completed with warnings', data.success ? 'success' : 'warning');
            ...
            updateOutputs(data) {
                ...
                if (data.stages?.length) {
                    let html = '<h3 style="color:#64ffda;">R Pipeline</h3>';
            ...
            reset(){
                ...
                document.getElementById('source-code').value = `# R is interpreted - source code → bytecode → R VM execution\nx <- 10\ny <- 20\nresult <- x + y\n\nprint(paste("Result:", result))\n\nfor (i in 1:5) {\n    print(paste("Iteration", i))\n}\n\nif (result > 25) {\n    print("Result is greater than 25")\n} else {\n    print("Result is 25 or less")\n}`;
                ...
                document.getElementById('stage-info').innerHTML = `<p><i class="fas fa-mouse-pointer"></i> R is an interpreted language: source → bytecode → R VM execution.</p><div style="margin-top:12px; padding:12px; background:rgba(100,255,218,0.05); border-radius:5px; border:1px dashed rgba(100,255,218,0.2);"><p style="color:#64ffda;"><i class="fas fa-lightbulb"></i> <strong>Key Distinction:</strong></p><p style="font-size:0.8rem; color:#8892b0;">Unlike compiled languages (C, Go), R code is first compiled to bytecode then executed by the R Virtual Machine.</p></div>`;
                this.clearScene(); this.createDefaultVisualization(); this.updateProgress(0); this.showStatus('Ready to interpret R code');
            }
            ...
            takeScreenshot(){ let link=document.createElement('a'); link.href=this.renderer.domElement.toDataURL('image/png'); link.download=`r_viz_${Date.now()}.png`; link.click(); }
            async download(type){
                ...
                try{
                    ...
                    a.href=url; a.download=type==='bytecode'?`bytecode_${this.sessionId}.rbc`:type==='ast'?`ast_${this.sessionId}.json`:`script_${this.sessionId}.r`;
                    document.body.appendChild(a); a.click(); document.body.removeChild(a); URL.revokeObjectURL(url);
                    this.showStatus(`${type} downloaded`,'success');
                }catch(e){ this.showStatus('Download failed','error'); }
            }
            loadExample(exampleId){
                if (rExamples[exampleId]) {
                    document.getElementById('source-code').value = rExamples[exampleId];
                    this.showStatus(`Loaded R example: ${exampleId}`, 'success');
                }
        ...
        document.addEventListener('DOMContentLoaded',()=>{ window.rViz = new RInterpreterVisualizer3D(); });
    </script>
</body>
</html>